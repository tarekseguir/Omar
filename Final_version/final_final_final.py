...
data = {'gender':[],'emotion':[],'stress_level':[],'speech':[],'time':[]}
...
class Application(tk.Frame):
    ...
    def say_hi(self):
        ...
        while True :
            ...
            frame = imutils.resize(frame, width=720,height=1024)
            ...
            for faceBox in faceBoxes:
                ...
                try:
                    ...
                    cv2.putText(frame, theRealEmotion, (faceBox[0]+60, faceBox[1]-10),cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                    cv2.putText(frame, f'{gender}', (faceBox[0]-40, faceBox[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2, cv2.LINE_AA)
                except:
                    print("your are too close")
            
            cv2.imshow("Frame", frame)
            k+=1
            if(k>20):
                try:
                    df.loc[j] = [gender,theRealEmotion,str(int(stress_value*100)),w,"{minutes}:{seconds}".format(minutes=minutes, seconds=seconds)]
                    print(df)
                except:
                    print("no face")
